<template>
	<view>
		...
		<view style="position: relative;height: 44px;background-color: #eae1c4;line-height: 44px;padding-left: 20rpx;">
			<text style="color:#dd7342 ;">等待医生回复</text>
		</view>
				...
				<view class="row" v-for="(item,index) in msgList" :key="index" :id="item.ID">
					...
					<block>
						...
						<view class="my" v-if="item.flow=='out'">
							...
							<!-- 右-头像 -->
							<view class="right">
								<image :src="item.avatar ? item.avatar : '/static/img/im/face/face_2.jpg'"></image>
							</view>
						</view>
						<!-- 别人发出的消息 -->
	...
	export default {
		...
		methods:{
			...
			getConversationList() {
				...
				let promise2 = this.tim.getFriendList();
				
				promise2.then(function(imResponse) {
				
				  const friendList = event.data; // 好友列表
