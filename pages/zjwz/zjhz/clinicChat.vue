<template>
	<view class="chat">
		<view class="noEnter" v-if="enterStatus == 0">
				 等待医生进入会诊群组
		</view>
		<view class="Enter" v-else>
				 <u-collapse :value="['2']">
						<u-collapse-item name="2">
							...
							<text class="bold">{{currentDoctor.realname}}</text>
							<text style="margin: 0 5px;">医生的会诊剩余</text>
							<text class="mainColor">
								{{currentDoctorTime}}
								<!-- <u-count-down
								        :time="30 * 60 * 60 * 1000"
								        format="DD:HH:mm:ss"
							...
							<text class="u-collapse-content">
								<view class="uni-flex uni-row">
									<view class="uni-flex uni-row" style="flex-wrap: wrap;">
										<view style="margin: 5px 10px;"  v-for="(item, index) in enterDoctor" :key="index">
											<u-avatar :src="item.face_image"></u-avatar>
											<text>{{item.realname}}</text>
										</view>
									</view>
									<!-- <view class="text flex" style="-webkit-flex: 1;flex: 1;flex-wrap: wrap;">
										<view style="margin: 5px 10px;"  v-for="(item, index) in enterDoctor" :key="index">
											<u-avatar :src="item.src"></u-avatar>
											<text>{{item.text}}</text>
										</view>
									</view> -->
									<view class="text" style="width: 150rpx;text-align: center; margin: 5px 0; 0 0">
										<view>
											<uni-icons type="plusempty" color="#000" size="16" />
										</view>
										<view>邀请更多</view>
									</view>
								</view>
	...
	<!-- 快捷工具 -->
	<view class="uni-flex uni-row tool" style="-webkit-justify-content: center;justify-content: center;">
		<uni-tag :circle="true"  text="发送病例" />
		<uni-tag :circle="true"  text="诊断建议" />
		<uni-tag :circle="true"  text="评价医生" @click="evaluate"/>
		<uni-tag :circle="true"  text="结束对话" @click="endChat"/>
	</view>
	<u-toast ref="uToast"></u-toast>
	<u-modal
		title="提示"
		content="切换其他医生,需结束当前医生对话,是否继续进行其他医生付费流程?"
		:show="endChatShow"
		showCancelButton
		closeOnClickOverlay
		@confirm="confirm"
...
<script>
	...
	export default {
		data() {
			return {
				enterStatus:1,
				currentDoctorTime:"5天12小时13分01秒",
				enterDoctor:[
					
				],
				timeData: {},
				order:false,endChatShow:false,
				//TIM变量
		...
		onLoad(option) {
			...
			this.group = JSON.parse(uni.getStorageSync('group'))
		...
		methods:{
			...
			confirm() {
				...
				if(g){
					this.dismissGroup(JSON.parse(g).groupID)
					uni.removeStorageSync("group")
				}
				// console.log('confirm');
				this.endChatShow = false
				uni.navigateTo({
					url: '/pages/zjwz/jswz/select-doctor'
				});
			},
			cancel() {
				console.log('cancel');
			...
			async getDoctor() {
				let doctor = this.doctorId.split(",")
				if(doctor.length > 0){
					for(let i = 0; i < doctor.length;i++){
						const res = await this.$axios({
						  url: expertUrl.doctorDetail + '?id=' + doctor[i]
						})
						this.enterDoctor.push(res.data.data)
					}
					console.log(this.enterDoctor)
					this.$store.commit('setCurrentDoctor', this.enterDoctor[0])
...
<style lang="scss">
	...
	.chat{
		height: 100%;
		.noEnter{
			background-color: #EAE1C4;color: #d97342;height: 50px;position: relative;
			padding: 0 10px;line-height: 50px;width: 100%;
		}
		.Enter{
		...
		.tool{
			position: fixed;z-index: 100;width: 100%;height: 48px;bottom: 48px;left: 0;text-align: center;
			margin: 0 auto;
			.uni-tag--default{
				border: 1px solid #cacdd1;margin: 0 1rpx;
			}
		}
	}
