	...
	<view class="jswz">
		...
		<view class="uni-list-cell picker-list">
			...
			<view class="uni-input-wrapper">
				<input class="uni-input picker-input" placeholder="请输入" v-model="this.patient.name" placeholderStyle="text-align:right"/>
			</view>
		</view>
		<view class="uni-list-cell picker-list">
			<view class="uni-list-cell-left">
			...
			<view class="uni-input-wrapper">
				<input class="uni-input picker-input" placeholder="请输入" v-model="this.patient.gender" placeholderStyle="text-align:right"/>
			</view>
		</view>
		<view class="uni-list-cell picker-list">
			<view class="uni-list-cell-left">
			...
			<view class="uni-input-wrapper">
				<input class="uni-input picker-input" name="patientAge" placeholder="请输入" v-model="this.patient.age" placeholderStyle="text-align:right"/>
			</view>
		</view>
		<view class="uni-list-cell picker-list">
		...
		<view class="text" style="padding-left: 15px;">备注</view>
		<view class="uni-textarea">
			<textarea style="height: 100px; padding-left: 15px;" name="remark" v-model="remark" placeholder="请填写"/>
		</view>
		</view>
		
		<view class="base-info">
		<view class="text" style="margin-top: 15px;padding-left: 15px;">病情描述</view>
		<view class="uni-textarea">
			<textarea style="height: 150px; padding-left: 15px;" name="caseDiscribe" v-model="caseDiscribe" placeholder="请填写"/>
		</view>
		<view class="text" style="padding-left: 15px;">诊断建议</view>
		<view class="uni-textarea">
			<textarea style="height: 100px; padding-left: 15px;" name="suggest" v-model="suggest" placeholder="请填写"/>
		</view>
		<view class="uni-flex" style="width: 100%;">
			<view class="text" style="padding-left: 15px;">患处照片 最多上传9张</view>
<!-- 			<view class="uni-flex">
		...
		<view class="uni-flex" style="width: 100%;">
			<view class="text" style="width: 30%; padding-left: 15px;">病例原件</view>
		</view>
		<view class="uni-uploader-body" style="padding-left: 15px;">
			<view class="uni-uploader__files">
				<block v-for="(image,index) in caseImageList" :key="index">
					<view class="uni-uploader__file">
						<image class="uni-uploader__img" :src="image" :data-src="image" @tap="previewCaseImage"></image>
					</view>
				</block>
				<view class="uni-uploader__input-box">
					<view class="uni-uploader__input" @tap="chooseCaseImage"></view>
				</view>
			</view>
		</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			// console.log("uploadCase-----options: ", options)
		...
		{	
			...
			chooseCaseImage: async function() {
				// #ifdef APP-PLUS
				// TODO 选择相机或相册时 需要弹出actionsheet，目前无法获得是相机还是相册，在失败回调中处理
				if (this.sourceTypeIndex !== 2) {
					let status = await this.checkPermission();
				...
				if (this.caseImageList.length === 9) {
					let isContinue = await this.isCasefullImg();
					console.log("是否继续?", isContinue);
					if (!isContinue) {
						return;
					}
				...
				uni.chooseImage({
					sourceType: sourceType[this.sourceTypeIndex],
					sizeType: sizeType[this.sizeTypeIndex],
					count: this.caseImageList.length + this.caseCount[this.caseCountIndex] > 9 ? 9 - this.caseImageList.length : this.caseCount[this.caseCountIndex],
					success: (res) => {
						this.caseImageList = this.caseImageList.concat(res.tempFilePaths);
					},
					fail: (err) => {
						// #ifdef APP-PLUS
						if (err['code'] && err.code !== 0 && this.sourceTypeIndex === 2) {
			...
			previewCaseImage: function(e) {
				var current = e.target.dataset.src
				uni.previewImage({
					current: current,
					urls: this.caseImageList
				})
			},
			
			async addPatientCase(){
				const data = {
					patient_id:this.patient.patientId,
					user_name :this.patient.name,
					gender :this.patient.gender,
					age :this.patient.age,
					atime :this.consulationDate,
					hospital_id:this.selectHospital,
					section_id:this.selectSection,
					remarks:this.remark,
					msg:this.caseDiscribe,
					suggest:this.suggest,
					affected_img:this.imageList,
					case_img:this.caseImageList,
				}
				console.log("--------addPatientCase data: ",data)
				try{
					const res = await this.$axios({
						method: 'post',
						url: expertUrl.addPatientCase,
						data
					})					
				}catch(e){
					if(e.code !== 1) {
				...
				uni.navigateTo({
					url: `quick-consultation?age=${age}&gender=${gender}&name=${name}&patientId=${patientId}&hasUploadCase=`+true
				});
			},
