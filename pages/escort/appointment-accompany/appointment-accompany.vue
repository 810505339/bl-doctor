<template>
  <view class="appointment-accompany">
    <escortTab :msgList="appointmentList"></escortTab>
    <escortActive :price="price" :url="'/pages/escort/appointment-accompany/appointment-accompany'"></escortActive>
    <escortManager :visible.sync="visible" :rightText="managerPrice"></escortManager>
    <escortNote :remarks.sync="remarks"></escortNote>
    <escortPay @payMoneyHandle="payMoneyHandle" :active="active" :money="payMoney"></escortPay>
	...
	export default {
		...
		methods:{
			...
			async payMoneyHandle() {
				const patient_id = this.appointmentList[0].user_id
				const hospital_id = this.appointmentList[1].hospital_id || 12
				const title = this.appointmentList[2].title
				const data = {
					type: 3, // 下单类型 (1 即时单 2 代办 3 预约单)
					patient_id, // 患者id
					hospital_id, // 医院id
					atime: title, // 预约时间
